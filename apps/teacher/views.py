...
def course_info(request, id):
    ...
    content = {'class_manage_course': 'layui-this',
               ...
               'class_search_user': '',
               'class_profile': '',
               'cour_name': cour_obj.CourseObj.cour_name,
               'sect_number': sect_obj.find_by_course(cour_id=id)['count'],
               'stu_number': NepLearnStatus.objects.filter(course_id=id).count(),
               'cour_status': '已激活' if cour_obj.CourseObj.cour_avilable else '未激活',
               'cour_status_color': '#009688' if cour_obj.CourseObj.cour_avilable else '#ffb53e',
...
def get_stu_by_cour(request, id):
    return JsonResponse(get_learn_list(course_id=id))
...
def delete_section(request):
    print(request.POST)
    sect_obj = Section(id=request.POST['sect_id'])
